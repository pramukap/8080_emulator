...
node *AddNode(char* new_label, uint16_t new_address, node* head)
{
...
uint32_t FindAddress(char *label_of_address, node *head)
{
...
//	token 	*program;
	...
	//receive and store tokens from assembly source
...
//	buffer = malloc(sizeof(char));
	...
	while((c = fgetc(assembly_file)) != EOF)
	{
		...
		if((program = realloc(program, (size + 1) * sizeof(char)) == NULL)
		{
			printf("Assembly Failed. Please try again.\n");
			exit(0);
		}
